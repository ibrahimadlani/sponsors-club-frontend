...
export default function ExplorePage() {
  ...
  const isExplorer = pathname === "/explore" || ["/explorer", "/athletes", "/teams", "/organisations"].some((p) => pathname.startsWith(p));
        ...
        >
          {user ? (
            ...
            <div className="flex justify-center sm:justify-around gap-6 sm:gap-6 w-full">
              ...
              <Link href="/explore" className={`flex flex-col items-center gap-0.5 font-medium w-full max-w-[56px] ${isExplorer ? 'text-pink-500' : 'opacity-70'}`}>
                <Search className="w-6 h-6" strokeWidth={isExplorer ? 2.5 : 1.5} />
                <span className={`text-[0.625rem] ${isExplorer ? 'font-bold' : 'font-medium'}`}>Explorer</span>
              </Link>
